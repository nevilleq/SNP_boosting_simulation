...
Read in optimal parameters and run some CV diagnostics.
...
#### Diagnose models with CV by accuracy and ROC  
...
auc_ggplot <- function(obs_level) {
  ...
  geom_point(data = res.long %>%
             ...
             size = 0.8,
             position = "jitter") +
#  geom_line(data = res.mean %>%
#                   filter(metric %in% "auc") %>%
#                   filter(n_obs %in% "100 Observations"),
...
#            size = 1) +
  ...
  labs(
    ...
    title = "Model CV AUC by Correlation, Method, and SNP's"
  ) +
...
#Generate Grobs
auc.gg.list <- map(.x = levels(res.mean$n_obs), ~auc_ggplot(.x))
...
```{r}
error_ggplot <- function(obs_level) {
  ggplot() +
  geom_point(data = res.long %>%
...
#            size = 1) +
  ...
  labs(
    x = "Correlation",
    y = "CV Error",
    title = "Model CV Error by Correlation, Method, and SNP's"
  ) +
  facet_wrap(~method + n_vars, ncol = 3) +
  scale_colour_viridis_d("Model") +
  scale_fill_viridis_d("Model")
...
#Generate Grobs
error.gg.list <- map(.x = levels(res.mean$n_obs), ~error_ggplot(.x))
